...
export default function InputField({
  ...
  const baseClasses = "peer w-full border border-border rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-emerald-600 dark:focus:ring-emerald-400";
    ...
    <div className="relative w-full">
      ...
      {placeholder && (
        <label
          htmlFor={fieldId}
          className={twMerge(
            "absolute left-3 top-2 text-muted-foreground pointer-events-none transition-transform duration-200",
            `peer-focus:text-xs peer-focus:-top-2 peer-focus:left-2
           peer-focus:text-emerald-600 peer-focus:bg-background peer-focus:px-1`,
            hasValue && "text-xs -top-2 left-2 text-muted-foreground bg-background px-1"
          )}
        >
          {placeholder}
        </label>
