...
export default async function CacheManagementPage() {
  ...
  const cachedDevices = await DeviceCacheManager.getAllDevices(session.session.activeOrganizationId!);
  const stats = await DeviceCacheManager.getStats(session.session.activeOrganizationId!);
  
  console.log("Cached Devices:", cachedDevices);
  console.log("Cache Stats:", stats);
  
  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold mb-4">Device Cache Management</h1>
